...
func newContainer(image string, name string, args []string) (*container, error) {
	...
	exec.Command("docker", "kill", "mediaserver-test-"+name).Run()
	exec.Command("docker", "wait", "mediaserver-test-"+name).Run()

	// --network=host is needed to test multicast
	cmd := []string{
		"docker", "run",
		"--network=host",
		"--name=mediaserver-test-" + name,
		"mediaserver-test-" + image,
	}
	cmd = append(cmd, args...)
...
func (c *container) close() {
	exec.Command("docker", "kill", "mediaserver-test-"+c.name).Run()
	exec.Command("docker", "wait", "mediaserver-test-"+c.name).Run()
	exec.Command("docker", "rm", "mediaserver-test-"+c.name).Run()
}

func (c *container) wait() int {
	exec.Command("docker", "wait", "mediaserver-test-"+c.name).Run()
	out, _ := exec.Command("docker", "inspect", "mediaserver-test-"+c.name,
