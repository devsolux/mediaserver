...
import (
	...
	"github.com/devsolux/mediaserver/internal/confwatcher"
	"github.com/devsolux/mediaserver/internal/externalcmd"
	"github.com/devsolux/mediaserver/internal/logger"
	"github.com/devsolux/mediaserver/internal/record"
	"github.com/devsolux/mediaserver/internal/rlimit"
)
...
var defaultConfPaths = []string{
	...
	"mediaserver.yml",
	"/usr/local/etc/mediaserver.yml",
	"/usr/etc/mediaserver.yml",
	"/etc/mediaserver/mediaserver.yml",
}
...
func New(args []string) (*Core, bool) {
	parser, err := kong.New(&cli,
		kong.Description("MediaServer "+version),
		kong.UsageOnError(),
		kong.ValueFormatter(func(value *kong.Value) string {
			switch value.Name {
			case "confpath":
				return "path to a config file. The default is mediaserver.yml."
...
func (p *Core) createResources(initial bool) error {
	...
	if initial {
		p.Log(logger.Info, "MediaServer %s", version)
