...
import (
	...
	"github.com/devsolux/mediaserver/internal/protocols/httpserv"
	"github.com/devsolux/mediaserver/internal/protocols/webrtc"
	"github.com/devsolux/mediaserver/internal/restrictnetwork"
)
...
func (s *webRTCHTTPServer) checkAuthOutsideSession(ctx *gin.Context, path string, publish bool) bool {
	...
	if res.err != nil {
		if terr, ok := res.err.(*errAuthentication); ok {
			if !hasCredentials {
				ctx.Header("WWW-Authenticate", `Basic realm="mediaserver"`)
