...
import (
	...
	"github.com/devsolux/mediaserver/internal/protocols/rtmp/bytecounter"
	"github.com/devsolux/mediaserver/internal/protocols/rtmp/h264conf"
	"github.com/devsolux/mediaserver/internal/protocols/rtmp/message"
)
...
func TestReadTracks(t *testing.T) {
	...
	for _, ca := range []struct {
		...
		{
			"issue mediaserver/386 (missing metadata), video+audio",
			&format.H264{
				PayloadTyp:        96,
				SPS:               h264SPS,
				PPS:               h264PPS,
		...
		{
			"issue mediaserver/386 (missing metadata), audio",
			nil,
			&format.MPEG4Audio{
				PayloadTyp: 96,
				Config: &mpeg4audio.Config{
		...
		{
			"issue mediaserver/2232 (xsplit broadcaster)",
			&format.H265{
				PayloadTyp: 96,
				VPS:        h265VPS,
				SPS:        h265SPS,
		...
		{
			"issue mediaserver/2289 (missing videocodecid)",
			&format.H264{
				PayloadTyp: 96,
				SPS: []byte{
		...
		{
			"issue mediaserver/2352 (missing audio)",
			&format.H264{
				PayloadTyp:        96,
				SPS:               h264SPS,
				PPS:               h264PPS,
